...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    @endif
    <button wire:click="create()" class="bg-blue-500 hover:bg-blue-700 text-bold font-bold py-2 px-4 rounded mt-10">Create New Company</button>
    @if ($count_section>0)
        <div class="py-10">
                ...
                {{-- <table class="table-auto">
                    ...
                    <tbody>
                        @foreach($sections as $section)
                            <tr>
                                <td class="px-5 py-5 bg-bold text-sm @if (!$loop->last) border-gray-200 border-b @endif">
                                ...
                                <td class="px-5 py-5 bg-white text-sm @if (!$loop->last) border-gray-200 border-b @endif text-right">
                                    <div class="inline-block whitespace-no-wrap">
                                        <button wire:click="edit({{ $section->id }})" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Edit</button>
                                        <button wire:click="$emit('triggerDelete',{{ $section->id }})" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Delete</button>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                    ...
                    <div class="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div class="bg-white rounded-lg text-left overflow-hidden shadow-xl">
                            ...
                            <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                                    <label for="section_nameInput" class="block text-gray-700 text-sm font-bold mb-2">Section Name:</label>
                                    <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="section_nameInput" placeholder="Enter Section Name" wire:model="section_name">
                                    @error('section_name') <span class="text-red-500">{{ $message }}</span>@enderror
                                    </div>
                                </div>
                                </div>
                                <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
...
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        @this.on('triggerDelete', sectionId => {
            Swal.fire({
                title: 'Are You Sure?',
